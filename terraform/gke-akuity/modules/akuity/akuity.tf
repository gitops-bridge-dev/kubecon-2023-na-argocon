...
resource "akp_cluster" "gitops-bridge" {
  ...
  kube_config = {
    ...
    client_certificate     = "${base64decode(data.google_container_cluster.current.master_auth.0.client_certificate)}"
    client_key             = "${base64decode(data.google_container_cluster.current.master_auth.0.client_key)}"
    cluster_ca_certificate = "${base64decode(data.google_container_cluster.current.master_auth.0.cluster_ca_certificate)}"
  }
  name      = "${var.cluster.cluster_name}"
  namespace = "akuity"
  labels    = merge(var.cluster.addons)
  annotations = var.cluster.metadata
  spec = {
    description = "${var.cluster.cluster_name} cluster"
    data = {
      size = "small"
    }
  }
